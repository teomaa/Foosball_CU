...
parser.add_argument("--ghost_level", type=int, default=None, help="Ghost demo mode: fixed ghost level (0-6)")
parser.add_argument(
...
class OverheadTiledVideoRecorder(gym.Wrapper):
    """Records a video of one random env's overhead TiledCamera (the camera
    that feeds the policy) whenever the main RecordVideo wrapper is also
    recording. Driven by the same step_trigger / video_length."""

    def __init__(self, env, video_folder, step_trigger, video_length, fps=30):
        super().__init__(env)
    ...
    def _get_cam(self):
        return getattr(self.env.unwrapped, "tiled_camera", None)
    ...
    def step(self, action):
        ...
        if not self._recording and self._step_trigger(self._step_id):
            num_envs = self.env.unwrapped.scene.num_envs
            self._env_idx = random.randint(0, num_envs - 1)
            self._frames = []
            self._recording = True
            self._record_start = self._step_id

        if self._recording:
            rgb = cam.data.output["rgb"][self._env_idx]
            if rgb.is_floating_point():
                frame = (rgb.clamp(0.0, 1.0) * 255.0).to(torch.uint8)
            else:
                frame = rgb.clamp(0, 255).to(torch.uint8)
            self._frames.append(frame.cpu().numpy())

            if len(self._frames) >= self._video_length:
                self._write_mp4()
                self._recording = False
                self._frames = []
    ...
    def _write_mp4(self):
        ...
        try:
            ...
            print(f"[WARN] Failed to write overhead video {path}: {e}")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # ghost opponent curriculum / demo settings
    ...
    if args_cli.ghost_level is not None and hasattr(env_cfg, "ghost_level"):
